...
C
       IF (OPTION.EQ.'COMP' .OR. OPTION.EQ.'ALL') THEN
         ...
         WRITE(IUNIT,3)IWMAX,MPBLK,NPMAP,SCALEF
         IF (IABS(ISWD).EQ.1 .OR. IABS(ISWD).EQ.2) THEN
           IZMIN=NINT(ZMIN)
           IZMAX=NINT(ZMAX)
           IZERO=NINT(ZEROL)
           WRITE(IUNIT,4)IZERO
           WRITE(IUNIT,6)'Data minimum',IZMIN,RMIN,IUZMIN,IVZMIN
           WRITE(IUNIT,6)'Data maximum',IZMAX,RMAX,IUZMAX,IVZMAX
         ELSE
           WRITE(IUNIT,5)ZEROL
           WRITE(IUNIT,7)'Data minimum',ZMIN,RMIN,IUZMIN,IVZMIN
           WRITE(IUNIT,7)'Data maximum',ZMAX,RMAX,IUZMAX,IVZMAX
         ENDIF
         WRITE(IUNIT,8)TYPE(IABS(ISWD))(1:LT),EXUNIT(1:LU)
         IF (ISWD.LT.0) THEN
           WRITE(IUNIT,9)IPMAP
         ENDIF
    1    FORMAT('0Computing redtape'/X,17('-'))
    2    FORMAT(' Map dimensions : ',2X,2I6/
     :          ' U-range        : ',2X,2I6/
     :          ' V-range        : ',2X,2I6)
    3    FORMAT(' Multiplicity   : ',8X,I6/
     :          ' Length of row  : ',8X,I6,' bytes'/
     :          ' Length of map  : ',8X,I6,' pages'/
     :          ' Scale factor   : ',1PE14.4)
    4    FORMAT(' Zero level',4X,' : ',I14)
    5    FORMAT(' Zero level',4X,' : ',1PE14.4)
    6    FORMAT(X,A,2X,' : ',I14,1PE15.4,' ext.units',
     :                                      ',  at u,v',2I5)
    7    FORMAT(X,A,2X,' : ',1PE14.4,E15.4,' ext.units',
     :                                      ',  at u,v',2I5)
    8    FORMAT(' Internal data type is',X,A,
     :           ',  external units are',X,A)
    9    FORMAT(' Indexed map, number of non-blank data values',I6)
         IF (io_attn(STATUS)) RETURN
       ENDIF
C
...
C
       IF (OPTION.EQ.'ASTRO' .OR. OPTION.EQ.'ALL') THEN
         ...
         LN=chr_lenb(NAME)
         WRITE(IUNIT,10)
         WRITE(IUNIT,11)NAME(1:LN)
         WRITE(IUNIT,12)IPOLN,POLN(1:LP)
         WRITE(IUNIT,13)FREQ
         WRITE(IUNIT,14)NSPAC
   10    FORMAT('0Astronomical redtape'/X,20('-'))
   11    FORMAT(' Map of ',A)
   12    FORMAT(' Polarisation   : ',8X,I6,X,A)
   13    FORMAT(' Frequency      : ',F14.4,' MHz')
   14    FORMAT(' No.of spacings : ',8X,I6)
C
         PRSEC=2
...
C          Data is valid mapping data.
           WRITE(IUNIT,15)
   15      FORMAT('0Map making redtape'/X,18('-'))
...
C          Map type.
     ...
     :             '          v: ', nvset, dvset, 'v= ', v0set
             ...
             ENDIF
             IF (BTEST(BSETFW,1)) WRITE(IUNIT,'(X,A,e10.3)')
     :            'Bandwidth smearing applied for (hz):',
     :            effective_bandwidth
             IF (BTEST(BSETFW,2)) WRITE(IUNIT,'(X,A)')
     :            'Primary beam correction applied.'
             IF (BTEST(BSETFW,3)) WRITE(IUNIT,'(X,A,f8.1)')
