...
*+SCNMAP2
       ...
       SUBROUTINE SCNMAP2 (REDTAPE, DATA, IUV, ZMNX, IZMNX, STATUS)
C      ------------------------------------------------------------
C
...
*-
       ...
       INTEGER    REDTAPE(MINIRT_LEN), IUV(4), IZMNX(4), STATUS
...
C  Check U,V range
C
       CALL CHCKUV2(MINIRT,IUV,STATUS)
       IF (STATUS.EQ.0) THEN
C
         IU1=IUV(1)
         IU2=IUV(2)
...
C
         NW=1
         IF (DTP_MRT.EQ.4) NW=2
C
C  Scan the U,V window, accumulating max, min, and data sums
C
         N=0
         DO IV=IV1,IV2,-1
           IX=((V1_MRT-IV)*NX_MRT+(IU1-U1_MRT))*NW-NW+1
           DO IU=IU1,IU2
             IX=IX+NW
             RDATA=DATA(IX)
             IF (RDATA.NE.BLK_MRT) THEN
...
C
       ENDIF
C
       IF (STATUS.NE.0) CALL MAPERR(STATUS,'in routine SCNMAP2')
C
       END
